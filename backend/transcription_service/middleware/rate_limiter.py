...
def _build_redis_uri() -> str:
    """
    Build Redis URI from config settings.
    Uses separate DB (db+1) for rate limiting to avoid key conflicts.
    """
    if settings.redis_password:
        return f"redis://:{settings.redis_password}@{settings.redis_host}:{settings.redis_port}/{settings.redis_db + 1}"
...
def _create_limiter() -> Limiter:
    ...
    try:
        # Try to create limiter with Redis storage
        ...
        logger.info(f"Rate limiter using Redis storage: {settings.redis_host}:{settings.redis_port}/{settings.redis_db + 1}")
        return rate_limiter
